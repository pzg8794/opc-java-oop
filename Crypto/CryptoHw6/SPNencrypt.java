...
// SPNencrypt: SPN encryption
public class SPNencrypt {
	...
	private SPNtables tab; // all the tables needed for SPN
	private byte[] w; // the expanded key

	// SPNencrypt: constructor for class.  Mainly expands key
	public SPNencrypt(byte[] key, int NkIn) {
		Nk = NkIn; // words in a key, = 4, or 6, or 8
		Nr = Nk + 6; // corresponding number of rounds
		tab = new SPNtables(); // class to give values of various functions
		w = new byte[4*Nb*(Nr+1)]; // room for expanded key
		KeyExpansion(key, w); // length of w depends on Nr
	}

	// Cipher: actual SPN encrytion
