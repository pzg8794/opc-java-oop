...
public class Card implements Comparable<Object>{
    ...
    */
	...
	public int compareTo(Object e) {
		...
		if((int)Mysuit.charAt(0)<(int)suit.charAt(0))
		{
			return -1;
		}		
		else if((int)Mysuit.charAt(0)>(int)suit.charAt(0))
			{
				return 1;
			}
			else if((int)Mysuit.charAt(0)==(int)suit.charAt(0))
				{
					if(rank.getValue()<tmp.getRank().getValue())
					{
						return -1;
					}
					else if(rank.getValue()>tmp.getRank().getValue())
					{
						return 1;
					}
	...
	for (int i = 0; i <= cards; i++){
			...
			temp[i] = new Card(TestBinSet.ranks[r], TestBinSet.suits[s]);
       ...
       Card[] temp2 = new Card[cards];
	int x =  0;

	while( x != cards){

	switch( temp[x].suit.chartAt(0)){
