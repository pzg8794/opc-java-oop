...
public class cg1Shape extends simpleShape
{
	...
	public void makeCylinder (float radius, int radialDivisions, int heightDivisions)
	{
		...
		double radiousAngle=Math.toRadians(360.0/radialDivisions);
		double cylinAngle=radiousAngle;
		for(int i=0;i<radialDivisions;i++) {
			x1[0]=(float)(radius*Math.cos(cylinAngle));
			z1[0]=(float)(radius*Math.sin(cylinAngle));
			cylinAngle += radiousAngle;
			
			x=(float)(radius*Math.cos(cylinAngle));
			z=(float)(radius*Math.sin(cylinAngle));
			float tmp =  0.5f;
			addTriangle(0.0f,tmp,0.0f,x1[0],tmp,z1[0],x,tmp,z);
			addTriangle(0.0f,-tmp,0.0f,x,-tmp,z,x1[0],-tmp,z1[0]);
			
			float frontSide[][]={{x1[0],tmp,z1[0]},{x1[0],-tmp,z1[0]},{x,-tmp,z},{x,tmp,z}};
			cubeShape.newVector = new Vector<float[]>();
			cubeShape.newVertrix = new Vector<float[]>();
			
			float allVertices[][]=(float[][])frontSide;
			for(int n=0;n<allVertices.length;n++) {
				float vertix1[]=allVertices[n%4];
				float vertix2[]=allVertices[(n+1)%4];
				double tmp1=1.0; 
				cubeShape.newVector.add(vertix1);
				
				for(int j=1;j<heightDivisions;j++) {
					tmp1=(1.0/heightDivisions)*j;
					setttingCoordinates(vertix1, tmp1, vertix2);
					cubeShape.newVector.add(pts);
				}
			}
			cubeShape.newVertrix.add((float[]) cubeShape.newVector.get(0));
			int rowFirst=cubeShape.newVector.size()-1;
			for(int n=0;n<heightDivisions;n++) {
				cubeShape.newVertrix.add((float[]) cubeShape.newVector.get(rowFirst--));
			}
			int counter=1;
			int counter1=cubeShape.newVector.size()-heightDivisions;
			counter1--;
			double tmp1=1.0;
	
			for(int m=1;m<heightDivisions;m++) {
				float vertix1[]=(float[])cubeShape.newVector.get(counter++);
				float vertix2[]=(float[])cubeShape.newVector.get(counter1--);
				cubeShape.newVertrix.add(vertix1);
				...
				cubeShape.newVertrix.add(vertix2);
			}
			int counter2 = heightDivisions;
			cubeShape.newVertrix.add((float[]) cubeShape.newVector.get(counter2++));
			...
			counter =0;
			counter1 = counter+heightDivisions+1;
			for (int i1=0;i1<heightDivisions;i1++) {
				int cnt1=counter, cnt2=counter1;
				for (int j=0; j<heightDivisions; j++) {
					settingPoints(cubeShape,cnt1, cubeShape,cnt2);
					addTriangle(x1[0],x1[1],x1[2],y1[0],y1[1],y1[2],pts[0],pts[1],pts[2]);
					cnt1++;
					addTriangle(y1[0],y1[1],y1[2],z1[0],z1[1],z1[2],pts[0],pts[1],pts[2]);
					cnt2++;
				}
				counter += heightDivisions+1; counter1=counter+heightDivisions+1;
			}
		}
	}
